...
```bash
CID=$(docker run -d --privileged -p 1194:1194/udp -p 443:443/tcp mircl/dockvpn)
docker run -t -i -p 8080:8080 --volumes-from $CID mircl/dockvpn serveconfig
```
...
When the `mircl/dockvpn` image is started, it generates:
...
Conveniently, `mircl/dockvpn` comes with a script called `serveconfig`,
...
- @jpetazzo/[dockvpn](https://github.com/jpetazzo/dockvpn)
  image based on the Ubuntu distribution
